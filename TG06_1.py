...
class FinancesForm(StatesGroup): # класс состояний для финансового бота
    category1 = State()
    expenses1 = State()
    category2 = State()
    expenses2 = State()
...
async def registration(message: Message):
   ...
   telegram_id = message.from_user.id  # получаем id юзера
   name = message.from_user.full_name  # получаем имя юзера

   await message.answer(f"Ваше имя: {name}, ваш id: {telegram_id}")
